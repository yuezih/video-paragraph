...
def compute(data):
  ...
  preds = {}
  i = 0
  for caption_id in data:
    candcaps[i] = [data[caption_id][0]['sentence']]
    ref_sentence = data[caption_id][0]['gt_sentence']
    refcaps[i] = [' '.join([c for c in ref_sentence])]
    preds[i] = data[caption_id][0]['sentence']
    i += 1
  # pdb.set_trace()
...
refs = json.load(open('/data2/yzh/recurrent-transformer/results/movie_mtrans_single_init_2022_06_06_16_06_22/greedy_pred_test.json'))
print(compute(refs['results']))