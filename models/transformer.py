...
class TransModel(framework.modelbase.ModelBase):
  ...
  def forward_loss(self, batch_data, TRG, step=None):
    ...
    outputs, key_enc, select = self.submods[DECODER](img_fts, trg_input, tags, src_mask, trg_mask, tags_mask, movie_id)
    # pdb.set_trace()
    ...
    if self.config.subcfgs[DECODER].rl:
      txt_ids, seqLogprobs, _ = self.submods[DECODER].sample(img_fts, tags, src_mask, decoding='sample')
  ...
  def evaluate(self, tst_reader):
    ...
    for batch_data in tqdm(tst_reader):
      ...
      output, _, attn = self.submods[DECODER].sample(img_fts, tags, img_mask, tags_mask, movie_id, decoding='greedy')
